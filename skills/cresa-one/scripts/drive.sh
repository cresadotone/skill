...
DRIVE_TOKEN="${CRESAONE_DRIVE_TOKEN:-}"
...
Global options:
  --api-key <key>        Account API key (or $CRESAONE_API_KEY / ~/.cresaone/credentials)
  --token <drv_live_...> Drive token (or $CRESAONE_DRIVE_TOKEN)
...
BASE_URL="${BASE_URL%/}"
if [[ "$BASE_URL" != "https://cresa.one" && "$ALLOW_NON_CRESAONE_BASE_URL" -ne 1 ]]; then
  if [[ -n "$API_KEY" || -n "$DRIVE_TOKEN" ]]; then
    die "refusing to send credentials to non-default base URL; pass --allow-noncresaone-base-url to override"
  fi
fi
...
if [[ -n "$DRIVE_TOKEN" ]]; then
  ...
  die "missing credentials; set CRESAONE_API_KEY, CRESAONE_DRIVE_TOKEN, or ~/.cresaone/credentials"
fi
